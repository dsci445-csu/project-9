...
* Our dataset used 20 (kills, gold, etc)
...
* 70/30 split between our train/test sets- using our small data set (n=250 matches)
* Built our GLM on the training set and applied to test data
* Result: 75% accuracy- seen from our confusion matrix
...
library(fastshap)
library(caret)
df <- read.csv("./sample data/d2_250_performance.csv")
df <- df[df$lane != "NONE", ]
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
cf <- confusionMatrix(data = glm_pred_class, reference = y_test)
...
## Fitting an SVM
* Works well with highly separated data (maximizes win and loss separation)
* Features: kills, deaths, dragonKills, baronKills, firstBloodKill, firstBloodAssist
* Reason: Predicted to be most influential on game outcome
...
We can extract things like impact of deaths on game outcome:
```{r, echo=TRUE}
