...
The data set that we were given for the competition had a lot of NA values. We tried a couple different methods for dealing with the NA values in our data set. For our linear regression approach, there were a few steps in our process. First, we removed the variables that had more than 50% NA values. There were only 10 variables that fit this description, but they all had a large proportion of NA values and we felt it would be best if we excluded those variables. Then, we removed all observations that had more than 50% NA values. There were 1097 variables removed this way, with a surprising large number of those observations having more than 90% NA values. Finally, we used imputation to fill in the remaining NA values.
$\\$
