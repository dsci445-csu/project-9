...
# graph
xgrid <- expand.grid(dragonKills = seq(from = min(X$dragonKills), to = max(X$dragonKills),
                                 length.out = 200),
                     baronKills = seq(from = min(X$baronKills), to = max(X$baronKills),
                                  length.out = 200))
...
# scale grid
grid_scaled <- scale(xgrid,
                     center = attr(X_train_scaled, "scaled:center"),
                     scale = attr(X_train_scaled, "scaled:scale"))
# predict with scaled grid
xgrid_copy$ygrid_pred <- predict(svm_model, newdata = grid_scaled)
xgrid_copy$ygrid_pred <- factor(xgrid$ygrid_pred, levels = levels(y_train))
# xgrid$pred_num <- as.numeric(xgrid$ygrid_pred)

# add ygrid to xgrid
# xgrid$ygrid_num <- ifelse(ygrid == "TRUE", 2, 1)
# xgrid$ygrid_fact <- factor(xgrid$ygrid_num)
...
ggplot() +
  ...
  geom_jitter(data = df, aes(x = dragonKills, y = baronKills, color = win)) +
  scale_fill_manual(values = c("False" = "red", "True" = "black"), name = "Predicted") +
  scale_colour_manual(values = c("False" = "red", "True" = "black"), name = "Actual") +
  theme_minimal() +
