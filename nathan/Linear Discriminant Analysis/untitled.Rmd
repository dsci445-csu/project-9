...
# Predicting Problematic Internet Use with LDA

The provided dataset, 'train.csv', contained a large amount of NA values in all variables.
...
# A preview of the data (First 20 rows of columns 2 - 10):

```{r, echo = FALSE, eval = TRUE}
x 
```

## LDA with Full Imputation

The first method I used was to impute all the NA values in the dataset.
...
print("Confusion Matrix on Train (Full Imputation)")
kable(confusion_matrix)

# Calculate accuracy
accuracy = sum(diag(confusion_matrix)) / sum(confusion_matrix)
print(paste("Accuracy (Train, Full Imputation):", round(accuracy * 100, 2), "%"))
...
print("Confusion Matrix (Test, Full Imputation):")
kable(confusion_matrix)

# Calculate accuracy
accuracy = sum(diag(confusion_matrix)) / sum(confusion_matrix)
print(paste("Accuracy (Test, Full Imputation):", round(accuracy * 100, 2), "%"))
...
## LDA when Removing NA from 'sii' Early

I noticed that in rows where `sii` was NA, many of the other cells in the row were also NA. So, here, I started by removing the rows where `sii` = NA. After doing so, R gave me an error that some predictors were colinear, so I removed the colinear columns before fitting the model.
...
print("Confusion Matrix (Train, NA Removal):")
kable(confusion_matrix)

# Calculate accuracy
accuracy = sum(diag(confusion_matrix)) / sum(confusion_matrix)
print(paste("Accuracy (Train, NA Removal):", round(accuracy * 100, 2), "%"))
...
print("Confusion Matrix (Test, NA Removal):")
kable(confusion_matrix)

# Calculate accuracy
accuracy = sum(diag(confusion_matrix)) / sum(confusion_matrix)
print(paste("Accuracy (Test, NA Removal):", round(accuracy * 100, 2), "%"))
